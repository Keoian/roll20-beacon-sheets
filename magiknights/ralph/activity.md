...
*Note: this session ran in parallel with the remote branch's own execution of
the same plan (sessions below). The merge kept the remote implementation and
re-applied this session's SRD corrections on top; the SRD compendium sync from
this session is authoritative.*
...
### 2026-07-17 — Merge with origin/main + SRD re-audit

Merged the local branch (upstream Roll20 merge + SRD compendium sync) with
origin/main's parallel implementation. Kept origin's implementation (superset:
tasks 18-37, hydration test suite, layout overhauls) and re-synced
`compendium/` from C:\git\magi-knights-srd (origin's copy was the older
pre-sync compendium). Then audited origin's code against the updated SRD and
patched these deviations:

- Re-added Finesse and Vicious weapon qualities (real SRD qualities that had
  been removed as "fabricated").
- Endurance Die now actually works per the SRD: the d6 is pre-rolled, the
  Stress/Exhaustion level is applied as a numeric penalty when the die fails
  to negate it (previously the d6 was summed into the roll total and no
  penalty existed), and it still rolls at level 6 (a 6 negates the -6 penalty;
  the Disadvantage remains).
- Poisoned added to attack-Disadvantage conditions; Horrified added to
  skill-check Disadvantage conditions.
- Condition effect texts corrected for Exposed, Paralyzed, Prone, Restrained,
  Silenced, and Soul-Siphoned I-III.
- Table 15-6: Massive size Attack Modifier corrected from -2 to -1.
- Level-ability descriptions replaced with SRD battle-technique text
  (Counter Blast, Perfect Parry, Extricate Aether, Heroic Resolve, Knight's
  Insight, Knight's Resolution, Swift Attack wording).
- Squire damage scaling corrected to the SRD table (1d6+3 / 2d6@5 / 3d6@10 /
  4d6@15).
- Soul Gun rolls: fixed DEX modifier lookup (was reading a nonexistent
  `abilityScores.dex`), and attachment Firing Pool bonuses now add to the pool
  total instead of adding extra d8s.
- Combat Forms: added "known" tracking; knowing Form X: Regulation (not
  mastering it) now grants Soul Gun access.
- Visor effects replaced with SRD text.
- Social bonds now use the SRD Bond Level tables (NPC: Build-a-Bond/Friendly/
  Caring/Devoted by SP; MK: through Mythic), derived from SP; NPC social view
